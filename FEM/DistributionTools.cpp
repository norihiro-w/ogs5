...
void getNodesOnDistribution(DistributionData &dis_data, MeshLib::CFEMesh &msh, std::vector<long> &nodes_vector)
{
	if (dis_data.geo_type == GEOLIB::POINT)
	{
		...
		if (node_id>=0) {
			...
			printf("-> node ID %d is found for POINT %s\n", node_id, dis_data.geo_name.c_str());
		}
	}
	else if (dis_data.geo_type == GEOLIB::POLYLINE)
	{
		printf("-> looking for nodes on POLYLINE %s\n", dis_data.geo_name.c_str());
	...
	{
		printf("-> looking for nodes on SURFACE %s\n", dis_data.geo_name.c_str());
...
//						std::cout << "\t" << k << "\t" << nodes_vector_old[k] << "\t" << msh_nod_vec[k] << std::endl;
	...
	if (!nodes_vector.empty() && dis_data.geo_type != GEOLIB::POINT)
		printf("-> %d nodes are found on the given distribution\n", nodes_vector.size());
}
